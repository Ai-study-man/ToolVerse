      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {favoriteTools.length > 0 && (
            <button
              ...
              className="px-4 py-2 text-sm text-red-200 border border-red-300/50 rounded-lg hover:bg-red-500/20 transition-colors"
            >
              Clear All
            </button>
          )}
        ...
        {favoriteTools.length === 0 ? (
          <div className="text-center py-16">
            <div className="w-24 h-24 mx-auto mb-6 text-white/30">
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" className="w-full h-full">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
              </svg>
            </div>
            <h3 className="text-xl font-medium text-white mb-2">No favorites yet</h3>
            <p className="text-white/70 mb-6">Start adding AI tools to your favorites to see them here</p>
            <a
              href="/"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {favoriteTools.map((tool) => (
              <div key={tool.id} className="bg-white/20 backdrop-blur-sm rounded-xl border border-white/20 overflow-hidden hover:bg-white/25 transition-all">
                <div className="p-6">
                  ...
                  <p className="text-sm text-white/80 mb-4 line-clamp-3">{tool.description}</p>
        ...
        {favoriteTools.length > 0 && (
          <div className="mt-8 p-4 bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg">
            <div className="flex items-start space-x-3">
              <svg className="w-5 h-5 text-accent-300 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <div>
                <h4 className="text-sm font-medium text-white">About Your Favorites</h4>
